 ...
 */
function getNodeInfo(hostName, ipAddress, consortiumid) {
  return new Promise(function (resolve, reject){
    ...
    Promise.all(web3PromiseArray).then(function(values){
      ...
      var nodeInfo = {hostname: hostName, peercount: peerCount, blocknumber: blockNumber, consortiumid: "codytest"+consortiumid};
      resolve(nodeInfo);
    });
  });
}
...
function getNodesInfo() {
  ...
  if (activeNodes.length > 0) {
    ...
    for(var i = 0; i < activeNodes.length; i++) {
      promiseArray.push(getNodeInfo(activeNodes[i].hostname, activeNodes[i].ipaddress, activeNodes[i].consortiumid));
    }
    ...
    Promise.all(promiseArray).then(function(values) {
      ...
      // sort in alphabetical order based on hostname
      nodeInfoArray = nodeInfoArray.sort(function(a, b) {
        var hostnameA = a.hostname.toUpperCase(); // ignore upper and lowercase
        var hostnameB = b.hostname.toUpperCase(); // ignore upper and lowercase
